...
class PuzzleService {
  ...
  private async getTotalCount(): Promise<number> {
    ...
    if (
      ...
      now - this.countCacheTime < this.CACHE_DURATION
    ) {
  ...
  public async getRandomPuzzle(): Promise<Puzzle | null> {
    ...
    const [puzzle] = await this.prisma.$queryRaw<Puzzle[]>`
   ...
   */
  public async getPuzzlesByTheme(
    theme: string,
    limit: number = 10,
  ): Promise<Puzzle[]> {
    return await this.prisma.puzzles.findMany({
   ...
   */
  public async getPuzzlesByOpening(
    opening: string,
    limit: number = 10,
  ): Promise<Puzzle[]> {
    return await this.prisma.puzzles.findMany({
   ...
   * Get popular puzzles
   */
  public async getPopularPuzzles(limit: number = 10): Promise<Puzzle[]> {
    return await this.prisma.puzzles.findMany({
      take: limit,
      orderBy: {
        Popularity: "desc",
  ...
  public getCacheStats() {
    ...
    return {
      ...
      cacheValid: this.totalCount !== null &&
        now - this.countCacheTime < this.CACHE_DURATION,
    };
  }
