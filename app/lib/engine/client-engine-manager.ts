/**
 * Client-side Chess Engine Manager
 * Manages Stockfish workers in the browser and communicates with server via SSE
 */
...
class ClientEngineManager {
  private static instance: ClientEngineManager;
  ...
  private maxWorkers = 2; // Reduced for client-side
  private cleanupInterval: NodeJS.Timeout;

  private constructor() {
    // Only initialize in browser
    if (typeof window === 'undefined') {
      throw new Error('ClientEngineManager can only run in browser environment');
    }
  ...
  static getInstance(): ClientEngineManager {
    if (typeof window === 'undefined') {
      throw new Error('ClientEngineManager can only run in browser environment');
    }

    if (!ClientEngineManager.instance) {
      ClientEngineManager.instance = new ClientEngineManager();
    }
    return ClientEngineManager.instance;
  }
  ...
  async startAnalysis(
    ...
    if (this.sessions.has(sessionId)) {
      await this.stopAnalysis(sessionId);
    }
    ...
    try {
      ...
      await this.sendWorkerCommand(worker, `setoption name Threads value 1`);
      await this.sendWorkerCommand(worker, `setoption name Hash value 128`);
   ...
   */
  private handleWorkerError(worker: StockfishWorker, error: Event | Error): void {
    if (worker.currentSession) {
      const session = this.sessions.get(worker.currentSession);
      if (session) {
        session.isActive = false;
  ...
  private cleanupIdleResources(): void {
    const now = Date.now();
    const workerIdleTimeout = 300000; // 5 minutes
    ...
    Array.from(this.workers.values()).forEach(worker => {
      if (!worker.isActive && !worker.currentSession && 
          (now - worker.createdAt) > workerIdleTimeout) {
        worker.worker.terminate();
        this.workers.delete(worker.id);
      }
    });
    ...
    Array.from(this.sessions.values()).forEach(session => {
      if (!session.isActive && (now - session.startedAt) > sessionTimeout) {
...
export default ClientEngineManager;